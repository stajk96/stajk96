...
def load_khz_schema(path: str | Path = SCHEMA_PATH) -> dict[str, Any]:
    """Load KHZ schema with explicit parser error context for easier troubleshooting."""
    try:
        with open(path, "r", encoding="utf-8") as f:
            return yaml.safe_load(f)
    except yaml.YAMLError as exc:
        raise ValueError(
            f"Invalid KHZ template schema YAML at '{path}'. Please fix syntax (e.g., quote special tokens like 'required?')."
        ) from exc
